...
X = featureset4 ############################# SELECTED FEATURESET ################################
y = np.array(df['cefr'])

## IMPORTANT VARIABLES
CLASSIFIER_NAME = "SVMP"
C, DEGREE = (1000, 6)
CLASSIFIER_PARAMETERS_NAMES = ['C', 'DEGREE']
CLASSIFIER_PARAMETERS = [C, DEGREE]
NREPEATS = 1000
TEST_SIZE = 0.2
...
for i in range(NREPEATS):
    print(i)
    results.append(pol_svm('exact'))
    results_off.append(pol_svm('off'))
...
def show_hist():
    ...
    plt.title(f"Histogram plot of accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions, C={C}, DEGREE={DEGREE}")
    plt.xlabel('Accuracy %')
    plt.show()

    plt.hist(results_off, edgecolor='black', bins=bins5)
    plt.title(f"Histogram plot of Off-by-one-level accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions, C={C}, DEGREE={DEGREE}")
    plt.xlabel('Accuracy %')
    plt.show()
...
all_results.to_csv(f"{CLASSIFIER_NAME} C={C} D={DEGREE} N={NREPEATS} split={TEST_SIZE}.csv")
