...
## IMPORTANT VARIABLES
CLASSIFIER_NAME = "Linear Regression"
CLASSIFIER_PARAMETERS = None
CLASSIFIER_PARAMETERS_NAMES = ['None']
CLASSIFIER_PARAMETERS = None
NREPEATS = 1000
TEST_SIZE = 0.2
...
def lr(kind):
    ...
    diff = np.round(y_prediction - y_test)
    diff = list(diff)
...
for i in range(NREPEATS):
    print(i)
    results.append(lr('exact'))
    results_off.append(lr('off'))
...
def show_hist():
    ...
    plt.title(f"Histogram plot of accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions")
    plt.xlabel('Accuracy %')
    plt.show()

    plt.hist(results_off, edgecolor='black', bins=bins5)
    plt.title(f"Histogram plot of Off-by-one-level accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions")
    plt.xlabel('Accuracy %')
    plt.show()
...
all_results.to_csv(f"{CLASSIFIER_NAME} N={NREPEATS} split={TEST_SIZE}.csv")
