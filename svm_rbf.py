...
## IMPORTANT VARIABLES
CLASSIFIER_NAME = "SVMR"
C, GAMMA = (1000, 'scale')
CLASSIFIER_PARAMETERS_NAMES = ['C', 'GAMMA']
CLASSIFIER_PARAMETERS = [C, 'scale']
NREPEATS = 1000
TEST_SIZE = 0.2
...
for i in range(NREPEATS):
    print(i)
    results.append(rbf_svm('exact'))
    results_off.append(rbf_svm('off'))
...
def show_hist():
    ...
    plt.title(f"Histogram plot of accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions, C={C}, GAMMA={GAMMA}")
    plt.xlabel('Accuracy %')
    plt.show()

    plt.hist(results_off, edgecolor='black', bins=bins5)
    plt.title(f"Histogram plot of Off-by-one-level accuracy of the {CLASSIFIER_NAME} classifier across {NREPEATS} training sessions, GAMMA={GAMMA}")
    plt.xlabel('Accuracy %')
    plt.show()
